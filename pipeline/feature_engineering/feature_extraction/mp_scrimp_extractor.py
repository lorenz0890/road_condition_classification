...
class MPScrimpExtractor(Extractor):
    ...
    def extract_features(self, data, args = None):
        ...
        """
        mp = matrixProfile.scrimp_plus_plus(data, args[0]) #6 or 32
        ...
        attr_vec = numpy.ndarray(shape=(sz, 3), dtype=float)
        #print(mtfs)
        count = 0
        i = 1.0
        for motif in mtfs:
            for index in motif: #['acceleration_abs', 'road_label']
                elem = numpy.array(data[args[4]].values[index:index + args[0]])
